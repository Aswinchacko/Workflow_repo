...
export default async function ProcurementListPage() {
  ...
  const isApprover = canApproveProcurement(user.role);
  ...
  return (
    <div className={cn("space-y-5", !isApprover && "lg:mx-auto lg:max-w-3xl")}>
      <PageHeader
        ...
        action={
          <Link href="/requests/procurement/new" className="block">
      ...
      <div className={cn("grid gap-5", isApprover && "lg:grid-cols-2 lg:items-start")}>
        {isApprover && (
          <Section title="Waiting for your review" empty="Nothing to review.">
            {pending.map((i) => (
              <Row
                key={i.id}
                href={`/requests/procurement/${i.id}`}
                title={i.item}
                subtitle={`${i.quantity} ${i.unit} · ${i.requester.name}`}
        ...
        <Section title={isApprover ? "Decided" : "My requests"} empty="No requests yet.">
          {(isApprover ? decided : items).map((i) => (
            <Row
              key={i.id}
              href={`/requests/procurement/${i.id}`}
              title={i.item}
              subtitle={`${i.quantity} ${i.unit} · ${i.project}`}
              status={i.status as RequestStatus}
            />
          ))}
        </Section>
